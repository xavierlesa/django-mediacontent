...
class AdminMediaContentWidget(AdminFileWidget):
    ...
    def render(self, name, value, attrs=None):
        ...
        if value and hasattr(value, "url"):
            ...
            #    output.append(u"<div style=\"overflow:hidden; width:100px; float:right\"><img src=\"%s\" /></div>" % (img_src[:-3] + img_src[-3:].replace('pdf', 'jpg')))
            else:
...
class MediaContentInline(generic.GenericStackedInline):
    ...
    fieldsets = (
        (None, {'fields': ('title', 'description', 'content', 'pub_date',)}),
    )
    formfield_overrides = { 
...
class MediaContentAdmin(admin.ModelAdmin):
    list_display = ('get_content', 'title', 'content_type', 'get_content_object', 'get_thumbnail')
    #list_display_links = ('get_content', 'title',)
    ...
    fieldsets = (
        (None, {'fields': ('title', 'description', 'content_type', 'object_pk', 'mimetype', 'content', 'thumbnail', 'gallery', 'pub_date')}),
    )
    ...
    def get_urls(self):
        ...
        my_urls = patterns('',
            url(r'^_ajax/(?:(?P<mimetype>[\w]+)/)?(?:(?P<pk>[\d]+)/)?$', self.admin_site.admin_view(csrf_exempt(AjaxMediaAPIView.as_view())), name='ajax_mediacontent_api'), 
        )
        return my_urls + urls
